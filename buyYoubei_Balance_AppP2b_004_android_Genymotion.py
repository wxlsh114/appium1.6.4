...
class AppP2b(unittest.TestCase):
    def setUp(self):
        ...
        desired_caps['app'] = PATH('51p2b_debug_2.4.3.apk')
    ...
    def buy_Youbei(self):
        ...
        mob=driver.find_element_by_class_name('android.widget.EditText')
        mob.click()
        #mob.send_keys("13711579877")
        mob.set_value('13711579877')
        sleep(2)
        ...
        driver.find_element_by_id('com.richfinancial.pujiaosuo:id/invest_pay_value').set_value('1900')
        sleep(2)
        #e-Bai
        ...
        #TouchAction(driver).press(x=975,y=1700).wait(500).release().perform()
        sleep(2)
        ...
        #TouchAction(driver).press(x=334,y=970).wait(500).release().perform()
        sleep(2)
        now=time.strftime('%Y-%m-%d %H_%M_%S')
...
if __name__ == '__main__':
    ...
    fp=open(filename,'wb')
    runner=HTMLTestRunner(stream=fp,title='51p2b of App environment android6.0[投资优贝(用账户余额支付)] test case report by Appium1.6.3',
